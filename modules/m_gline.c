		...
		const char *host, const char *reason)
{
	...
	ircsprintf(buffer, "%s - Data/Hora: %s", reason, smalldate_pt(cur_time));
       ...
       if( irccmp(host, target_p->sockhost) == 0 )
       {
        ...
        if(IsServer(client_p))
                sendto_one(target_p, ":%s KILL %s :%s",
                       client_p->name, target_p->name, "Você foi banido da UnIRC Network.");
        else
            sendto_one(target_p, ":%s!%s@%s KILL %s :%s",
                   client_p->name, client_p->username, client_p->host,
                   target_p->name, "Você foi banido da UnIRC Network.");
