...
check_client(va_list args)
{
    ...
    case TOO_MANY:
        sendto_one(source_p, ":%s NOTICE %s :*** IP: %s [%s]", MENAME(source_p), source_p->name, source_p->sockhost, source_p->realhost);
        sendto_one(source_p, ":%s NOTICE %s :*** Clones are forbidden on the RedeBrasil. If you need PROXY to connect on our network or to run allowed bots, visit: http://www.redebrasil.org.br/proxy/", MENAME(source_p), source_p->name);
	...
	if(aconf != NULL)
	{
			...
			|| (ConfigFileEntry.glines && IsConfGline(aconf)))
		{
			...
			if(ConfigFileEntry.kline_with_reason)
                sendto_one(client_p, ":%s NOTICE %s :*** IP: %s [%s]", MENAME(client_p), client_p->name, client_p->sockhost, client_p->realhost);
                sendto_one(client_p, ":%s NOTICE %s :*** You are banned from RedeBrasil. Reason %s", MENAME(client_p), client_p->name, aconf->reason);
                sendto_one(client_p, ":%s NOTICE %s :*** For more information, visit: http://www.redebrasil.org.br/akill/ ", MENAME(client_p), client_p->name);
