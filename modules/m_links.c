...
do_links(struct Client *source_p, int parc, char **parv)
{
	if(IsOper(source_p) || !ConfigServerHide.flatten_links)
	{
		...
		DLINK_FOREACH(ptr, global_serv_list.head)
		{
            ...
            // agora o server 0 sera sempre o irc.redebrasil.org.br :P
            // --XOOM
            if(target_p->hopcount == 0) {
                sendto_one(source_p, form_str(RPL_LINKS),
                       me_name, nick,
                       "irc.redebrasil.org.br", "irc.redebrasil.org.br", 0, "RedeBrasil de Servidores de IRC");
                sendto_one(source_p, form_str(RPL_LINKS),
                       me_name, nick,
                       target_p->name, "irc.redebrasil.org.br", target_p->hopcount+1, p);
            } else {
                sendto_one(source_p, form_str(RPL_LINKS),
                       me_name, nick,
