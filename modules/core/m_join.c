...
m_join(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
    ...
    if ((IsGringo(source_p)) && (MyClient(source_p))) {
        if(!(match(CANAL_AJUDA, parv[1]))) {
            sendto_one(source_p, ":%s NOTICE %s :You won't be able to send messages or join a channel while your nickname is not identified or registered. To register your nickname, type \002/NICKSERV REGISTER <PASS> <MAIL>\002 or type \002/join #Ajuda\002 for help", MENAME(source_p), source_p->name);
            return;
        }
    }
