...
verifica_gringo(struct Client *source_p)
{
  if(!(find_proxy(source_p->sockhost)) && !(find_proxy(source_p->realhost)))
  {
      if (!(find_ipbr(source_p->sockhost))) {
          sendto_one(source_p, ":NOTICE %s :You won't be able to send messages or join a channel while your nickname is not identified or registered. To register your nickname, type \002/NICKSERV REGISTER <PASS> <MAIL>\002 or type \002/join #Ajuda\002 for help", MENAME(source_p), source_p->name);
...
/*	if(IsCloaked(source_p))
		sendto_one(source_p, ":%s NOTICE %s :*** Seu host está encriptado (%s)", MENAME(source_p),
			   source_p->name, source_p->host);*/
