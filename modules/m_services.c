        ...
        for(m = *p; *m; m++)
        {
            ...
            case 'B' :
              if ((what == MODE_ADD) && (!IsRegNick(target_p)) ) {
                      //sendto_one(target_p, ":%s NOTICE %s :\24ATENÇÃO:\2 Você não poderá enviar mensagens ou entrar em canais enquanto não identificar seu nick ou registra-lo. Para mais informações, por favor digite \002/NICKSERV HELP\002", me.name, target_p->name);
