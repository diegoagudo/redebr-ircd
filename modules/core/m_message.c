	  ...
	  struct Client *source_p, int parc, char *parv[])
{
    ...
    if(!IsServer(source_p) && !IsServices(source_p) && (!IsOper(source_p) && MyClient(source_p)))
    {
      if(parv[2])
      {
          if(verifica_spamsg(parv[2])) { //by XOOM <xoom@redebrasil.org.br>
              //sendto_one(source_p,":%s NOTICE %s :Esta mensagem não pode ser enviada, pois contém palavras consideradas proibidas na RedeBrasil.", me.name, parv[0]);
